...
void CodeEdit::lineNumberAreaPaintEvent(QPaintEvent *event)
{
    ...
    painter.fillRect(QRect(0,0,40,1560),QColor(34,39,49));
    ...
    while (block.isValid() && top <= event->rect().bottom())
    {
        if (block.isVisible() && bottom >= event->rect().top())
        {
            ...
            painter.drawText(0, top, 40, fontMetrics().height(),Qt::AlignCenter, number);
        }
        block = block.next();
        top = bottom;
    ...
    if(!word.isEmpty()){//光标所在单词是不是合法(能不能联想)
        ...
        foreach(const QString &temp,completeList){
            if(temp.contains(word)){
                ...
                distance[temp]=CompleteListWidget::ldistance(temp,word);
        ...
        if(itemList.size()>0){//如果有的话
            std::sort(itemList.begin(),itemList.end(),[&](const QString &s1,const QString &s2)->bool{return distance[s1]<distance[s2]; });
...
int CodeEdit::getCompleteWidgetX()
{
    ...
    int x=this->cursorRect().x()+4*fontMetrics().width(QLatin1Char('9'));
