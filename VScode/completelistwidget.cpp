...
CompleteListWidget::CompleteListWidget(QWidget *parent):
    QListWidget(parent)
{
...
int CompleteListWidget::ldistance(const QString source, const QString target)
{
//step 1
    int n = source.length();
    int m = target.length();
...
//step 3
    for (int i = 1; i <= n; i++)
    {
        const QChar si = source[i - 1];
//step 4
        for (int j = 1; j <= m; j++)
        {
            const QChar dj = target[j - 1];
//step 5
            int cost;
            if (si == dj)
            {
